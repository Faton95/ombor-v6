...
import { createStatus } from 'src/actions/status';
...
export default function UserCreateForm({ open, onClose }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      toast.success('Sabab muvaffaqiyatli yaratildi!');
      onClose();
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    ...
    >
      <Form methods={methods} onSubmit={onSubmit}>
        <DialogTitle>Yangi sabab yaratish</DialogTitle>

        <DialogContent>
          <Box rowGap={3} columnGap={2} display="grid" sx={{ pt: 1 }}>
            <Field.Text name="name" label="Sabab" />
          </Box>
        </DialogContent>
